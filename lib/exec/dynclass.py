...
from ..data.dynclass import Brain10, DynCSL
...
def main(*ARGS):
    ...
    # Translate arguments.
    datasetize = (
        {"Brain10": Brain10, "DynCSL": DynCSL}[source]
    )
    winsize = {"Brain10": 12, "DynCSL": 8}[source]
    ...
    metaset = (
        dataset.asto_dynamic_adjacency_list_dynamic_edge(
            window_history_size=winsize, window_future_size=0,
    ...
    metaset.inputon(["none", "none", "all", "none"])
    metaset.targeton(["none", "none", "none", targeton])
    print(metaset)
    ...
    if len(resume_eval) == 0:
        ...
        framework.fit(
            ...
            batch_size=num_batch_graphs, max_epochs=max_epochs, validon=ERR,
            validrep="ERR", patience=patience,
        )
    else:
        #
        framework.besteval(
            (7, 1, 2), (2, 1, 0), train_prop_tuple,
            batch_size=num_batch_graphs, validon=ERR, validrep="ERR",
            resume=resume_eval,
        )